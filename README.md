...
These files prepare local discovery only. They do not install or publish the plugin, and they declare no MCP server or app. `publisher/publisher.json` derives the Openly Useful publisher identity and policy URLs from <https://openlyuseful.org/publisher/manifest.json>, whose version-controlled source is `publisher/manifest.json` in this repository. Each declared policy page has a version-controlled repository mirror (`PRIVACY.md`, `TERMS.md`, `SECURITY.md`, `SUPPORT.md`); the live pages and their repository mirrors are maintained as 1:1 copies. Openly Useful LLC remains a planned, formation-pending publisher/operator/licensee, and external publication authorization remains withheld.
